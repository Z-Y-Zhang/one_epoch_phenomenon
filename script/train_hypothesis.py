...
def cal_emb_acc(truth_data, false_data, label, label_test_emb):

    # default setting in sklearn 0.22.1
    lr_model = LogisticRegression(penalty='l2', dual=False, tol=1e-4, C=1.0,
     fit_intercept=True, intercept_scaling=1, class_weight='balanced',
     random_state=None, solver='lbfgs', max_iter=100,
     multi_class='auto', verbose=0, warm_start=False, n_jobs=None)

    #train set Linear Separability
    trainset_scores = cross_validate(lr_model, preprocessing.minmax_scale(truth_data), label, cv=5,  return_train_score=True)
    ...
    lr_model = LogisticRegression(penalty='l2', dual=False, tol=1e-4, C=1.0,
     ...
     random_state=None, solver='lbfgs', max_iter=100,
     multi_class='auto', verbose=0, warm_start=False, n_jobs=None)
    #test set Linear Separability
    testset_scores = cross_validate(lr_model, preprocessing.minmax_scale(false_data), label_test_emb, cv=5,  return_train_score=True)
...
def train(
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        test_emb_data_pool,_,_ = generator_queue(test_emb_data)
        src_test_emb,tgt_test_emb = test_emb_data_pool.get()
...
#                    continue
                ...
                ####train test emb
                if iter % 1 == 0:
                    truth_data = cal_dict['inp']

                    nick_id, item_id, cate_id, label_test_emb, hist_item, hist_cate, neg_item, neg_cate, hist_mask = prepare_data(src_test_emb, tgt_test_emb)
                    _, _, _, _, cal_dict_test = model.calculate(sess, [nick_id, item_id, cate_id, hist_item, hist_cate, neg_item, neg_cate, hist_mask, label_test_emb])
                    false_data = cal_dict_test['inp']

                    tmp_score_list = [[], []] # trainset testset
                    trainset_scores, testset_scores = cal_emb_acc(truth_data, false_data, label, label_test_emb)

                    tmp_score_list[0].append(np.mean(trainset_scores['test_score']))
                    tmp_score_list[1].append(np.mean(testset_scores['test_score']))
                    if iter % 50 == 0:
                    # if True:
                        print('iter', iter, "train %0.5f +- %0.5f" % (trainset_scores['test_score'].mean(), trainset_scores['test_score'].std()), "  test %0.5f +- %0.5f" % (testset_scores['test_score'].mean(), testset_scores['test_score'].std()))

                    for d in range(len(truth_data[0,:])//EMBEDDING_DIM):
                        trainset_scores,testset_scores = cal_emb_acc(truth_data[:,d*EMBEDDING_DIM:(d+1)*EMBEDDING_DIM], false_data[:,d*EMBEDDING_DIM:(d+1)*EMBEDDING_DIM], label, label_test_emb)
                        tmp_score_list[0].append(np.mean(trainset_scores['test_score']))
                        tmp_score_list[1].append(np.mean(testset_scores['test_score']))
                    # print('iter', iter, tmp_score_list)
                    hyp_list.append(tmp_score_list)
            ...
            print "\n\ntrain_auc_list=",train_auc_whole_list
            print "test_auc_list=",test_auc_list
            print "train_loss=",train_loss_whole_list
            print "test_loss=",test_loss_list


            print "hyp_list=",(np.around(hyp_list, decimals=5)).tolist()
